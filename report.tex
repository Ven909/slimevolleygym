...
\subsection{Agent C: Exploiter Design}

Agent C trains with full visibility against a frozen 10M-step Agent B. The hypothesis is that Agent C can learn to place the ball so that it crosses the net at moments and angles where Agent B, without ball visibility on the opponent's side, cannot anticipate its arrival. Training for 5M steps produced a model (151 KB) that was visually competitive during informal testing. A full three-way round-robin evaluation (Agent C vs.\ B, Agent A vs.\ B, Agent C vs.\ A) was implemented in \texttt{eval\_exploiter.py} but was not run to completion at the time of this report, so quantitative comparison of Agent C against the other agents is deferred to future work.
...
The main limitation of this evaluation is the draw-heavy scoring structure of Slime Volleyball. Even a clear performance gap between agents manifests only as a difference in score margin rather than in win rate, making it hard to demonstrate statistical significance with a modest number of episodes. A second limitation is that the exploiter's potential advantage over Agent B has not yet been quantified through the round-robin evaluation. Third, all training used default PPO hyperparameters; a systematic hyperparameter search could improve performance.
...
The 10M-step frame-stacked Agent B draws all 200 games against the baseline but concedes more points than it scores (score margin $-0.98$), indicating that the masking makes it harder to develop a dominant policy. The agent scores in some games (max reward $+2.0$) but concedes more often (min reward $-4.0$), suggesting it learned a partial strategy that can return the ball but does not proactively attack.
...
\begin{itemize}
    \item \textbf{Complete round-robin evaluation.} Running the three-way Agent A vs.\ B vs.\ C evaluation would directly test whether the exploiter hypothesis holds.
